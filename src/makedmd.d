...
enum DEFAULT_BIT_MODE = 32;
enum DEFAULT_DCOMPILER = "dmd";

__gshared ushort bitMode = DEFAULT_BIT_MODE;
__gshared string dCompiler = DEFAULT_DCOMPILER;

void usage()
{
    writeln ("Usage: makedmd [options] [target]");
    writeln ("  -h                show this help");
    writefln("  -32 | -64         compile in 32/64 bit mode (default=%s)", DEFAULT_BIT_MODE);
    writefln("  -dcompiler <exe>  specify the D compiler to use (default=%s)", DEFAULT_DCOMPILER);
}
static string nextArg(string[] args, size_t* argIndex)
{
    (*argIndex)++;
    if(*argIndex >= args.length)
    {
        writefln("Error: options '%s' requires an argument", args[(*argIndex) - 1]);
        exit(1);
    }
...
int main(string[] args)
{
    args = args[1..$];
    {
        ...
        for(size_t i = 0; i < args.length; i++)
        {
            ...
            if(arg.length == 0 || arg[0] != '-')
            {
                ...
                bitMode = 64;
            else if(arg == "-dcompiler")
                dCompiler = args.nextArg(&i);
            else if(arg == "-h")
            {
                usage();
                return 1;
    ...
    {
        writefln("Error: too many command line options");
        return 1;
    }
    
    // TODO: make output directory first
    string os = "windows";
    string build = "release";
    string model = bitMode.to!string;
    string outputDir = buildPath("..", "generated", os, build, model);
    if(!exists(outputDir))
    {
        writefln("mkdir %s", outputDir);
        mkdirRecurse(outputDir);
    }
    auto targetExe = buildPath(outputDir, "dmd" ~ exeExtension);

    static string dmdSource(string filename)
    {
        return "dmd" ~ dirSeparator ~ filename;
    }
    auto dmdFrontSource = [
        dmdSource("access.d"),
        dmdSource("aggregate.d"),
        dmdSource("aliasthis.d"),
        dmdSource("apply.d"),
        dmdSource("argtypes.d"),
        dmdSource("arrayop.d"),
        dmdSource("arraytypes.d"),
        dmdSource("astcodegen.d"),
        dmdSource("attrib.d"),
        dmdSource("builtin.d"),
        dmdSource("canthrow.d"),
        dmdSource("clone.d"),
        dmdSource("complex.d"),
        dmdSource("cond.d"),
        dmdSource("constfold.d"),
        dmdSource("cppmangle.d"),
        dmdSource("cppmanglewin.d"),
        dmdSource("ctfeexpr.d"),
        dmdSource("dcast.d"),
        dmdSource("dclass.d"),
        dmdSource("declaration.d"),
        dmdSource("delegatize.d"),
        dmdSource("denum.d"),
        dmdSource("dimport.d"),
        dmdSource("dinifile.d"),
        dmdSource("dinterpret.d"),
        dmdSource("dmacro.d"),
        dmdSource("dmangle.d"),
        dmdSource("dmodule.d"),
        dmdSource("doc.d"),
        ...
        dmdSource("dstruct.d"),
        dmdSource("dsymbol.d"),
        dmdSource("dsymbolsem.d"),
        dmdSource("dtemplate.d"),
        dmdSource("dversion.d"),
        dmdSource("escape.d"),
        dmdSource("expression.d"),
        dmdSource("expressionsem.d"),
        dmdSource("func.d"),
        dmdSource("hdrgen.d"),
        dmdSource("id.d"),
        dmdSource("imphint.d"),
        dmdSource("impcnvtab.d"),
        dmdSource("init.d"),
        dmdSource("initsem.d"),
        dmdSource("inline.d"),
        dmdSource("inlinecost.d"),
        dmdSource("intrange.d"),
        dmdSource("json.d"),
        dmdSource("lib.d"),
        dmdSource("link.d"),
        dmdSource("mars.d"),
        ...
        dmdSource("objc.d"),
        dmdSource("opover.d"),
        dmdSource("optimize.d"),
        dmdSource("parse.d"),
        dmdSource("sapply.d"),
        dmdSource("semantic.d"),
        dmdSource("sideeffect.d"),
        dmdSource("statement.d"),
        dmdSource("staticassert.d"),
        dmdSource("target.d"),
        dmdSource("safe.d"),
        dmdSource("blockexit.d"),
        dmdSource("permissivevisitor.d"),
        dmdSource("transitivevisitor.d"),
        dmdSource("parsetimevisitor.d"),
        dmdSource("printast.d"),
        dmdSource("typesem.d"),
        dmdSource("traits.d"),
        dmdSource("utils.d"),
        dmdSource("visitor.d"),
        dmdSource("libomf.d"),
        dmdSource("scanomf.d"),
        dmdSource("templateparamsem.d"),
        dmdSource("typinf.d"),
        dmdSource("libmscoff.d"),
        dmdSource("scanmscoff.d"),
        dmdSource("statement_rewrite_walker.d"),
        dmdSource("statementsem.d"),
        dmdSource("staticcond.d"),
    ];
    auto dmdGlueSource = [
        dmdSource("irstate.d"),
        dmdSource("toctype.d"),
        dmdSource("glue.d"),
        dmdSource("gluelayer.d"),
        dmdSource("todt.d"),
        dmdSource("tocsym.d"),
        dmdSource("toir.d"),
        dmdSource("dmsc.d"),
        dmdSource("tocvdebug.d"),
        dmdSource("s2ir.d"),
        dmdSource("toobj.d"),
        dmdSource("e2ir.d"),
        dmdSource("objc_glue_stubs.d"),
        dmdSource("eh.d"),
        dmdSource("iasm.d"),
    ];
    static string backendSource(string filename)
    {
        return "dmd" ~ dirSeparator ~ "backend" ~ dirSeparator ~ filename;
    }
    auto dmdBackHeaders = [
        backendSource("bcomplex.d"),
        backendSource("cc.d"),
        backendSource("cdef.d"),
        backendSource("cgcv.d"),
        backendSource("code.d"),
        backendSource("cv4.d"),
        backendSource("dt.d"),
        backendSource("el.d"),
        backendSource("global.d"),
        backendSource("obj.d"),
        backendSource("oper.d"),
        backendSource("outbuf.d"),
        backendSource("rtlsym.d"),
        backendSource("code_x86.d"),
        backendSource("iasm.d"),
        backendSource("ty.d"),
        backendSource("type.d"),
        backendSource("exh.d"),
        backendSource("mach.d"),
        backendSource("md5.d"),
        backendSource("mscoff.d"),
        backendSource("dwarf.d"),
        backendSource("dwarf2.d"),
        backendSource("xmm.d"),
    ];
    static string tkSource(string filename)
    {
        return "dmd" ~ dirSeparator ~ "tk" ~ dirSeparator ~ filename;
    }
    auto dmdTkHeaders = [
        tkSource("dlist.d"),
    ];
    auto dmdAllSource = chain(dmdFrontSource, dmdGlueSource, dmdBackHeaders, dmdTkHeaders);
    
    FileSystemMakeEngine make;
//$(TARGETEXE): $(DMD_SRCS) $(ROOT_SRCS) $G\newdelete.obj $(LIBS) $(STRING_IMPORT_FILES)
    make.rule()
        .target(targetExe)
        .dependRange(dmdAllSource)
        .action(delegate(Item target, Target[] ruleTargets, Item[] dependencies)
        {
            run(format("%s -of%s -vtls -J%s -J../res -L/STACK:8388608%s",
                dCompiler, target.getName, outputDir, dmdAllSource.formatArgRange
                ));
            //$(HOST_DC) -of$@ -vtls -J$G -J../res -L/STACK:8388608 $(DFLAGS) $(LFLAGS) $(DMD_SRCS) $(ROOT_SRCS) $G\newdelete.obj $(LIBS)
            //copy $(TARGETEXE) .
            writefln("TODO: need to copy need to make %s", targetExe);
        })
        ;
    make.rule()
        .target("default")
...
@property auto formatArgRange(T)(T range)
{
    static struct Formatter
    {
        ...
        void toString(scope void delegate(const(char)[]) sink)
        {
            foreach(item; range)
            {
                formattedWrite(sink, " %s", item.formatQuotedIfSpaces);
            }
        }
    }
